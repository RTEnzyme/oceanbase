                                          ...
                                          bool &trans_happened)
{
  ...
  UNUSED(parent_stmts);
  LOG_INFO("masd start to transform");
  if (OB_ISNULL(stmt) || OB_ISNULL(ctx_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_INFO("invalid argument", K(ret), K(stmt), K(ctx_));
    ...
    OPT_TRACE("not select stmt");
                                              ...
                                              is_valid))) {
    LOG_INFO("masd failed to check transform validity", K(ret));
  } else if (!is_valid) {
    //do nothing
    LOG_INFO("masd can not do min max transform");
  } else if (OB_FAIL(do_transform(static_cast<ObSelectStmt *>(stmt)))) {
    LOG_INFO("masd failed to transform column aggregate", K(ret));
  } else if (OB_FAIL(add_transform_hint(*stmt))) {
    LOG_INFO("masd failed to add transform hint", K(ret));
  } else {
    trans_happened = true;
  }
  LOG_INFO("masd End transform", K(ret), K(stmt));
  return ret;
}
                                                ...
                                                bool &is_valid)
{
  ...
  if (OB_ISNULL(select_stmt)) {
    ...
    LOG_INFO("unexpected null", K(ret), K(select_stmt));
...
int ObTransformMinMax::do_transform(ObSelectStmt *select_stmt)
{
  ...
  if (OB_ISNULL(select_stmt)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_INFO("params have null", K(ret), K(select_stmt));
  } else if (OB_FAIL(do_minmax_transform(select_stmt))) {
    LOG_INFO("masd failed to transform minmax", K(ret));
  }
  LOG_INFO("masd succeed", K(select_stmt));
  return ret;
}
...
int ObTransformMinMax::do_minmax_transform(ObSelectStmt *select_stmt)
{
  ...
  if (OB_ISNULL(select_stmt) || OB_ISNULL(ctx_) || OB_ISNULL(ctx_->expr_factory_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_INFO("params have null", K(ret), K(select_stmt), K(ctx_));
  } else {
    ObRawExprCopier copier(*ctx_->expr_factory_);
    if (OB_FAIL(ObTransformUtils::create_simple_view(ctx_, select_stmt, view_child_stmt))) {
      LOG_INFO("masd masd failed to create simple view", K(ret));
    } else if (OB_FAIL(select_stmt->get_column_exprs(old_exprs))) {
      LOG_INFO("masd failed to get column exprs", K(ret));
    } else if (OB_FAIL(view_child_stmt->get_select_exprs(new_exprs))) {
      LOG_INFO("masd failed to get select exprs", K(ret));
    } else if (OB_FAIL(copier.add_replaced_expr(old_exprs, new_exprs))) {
      LOG_INFO("masd failed to add replace pair", K(ret));
    } else if (OB_ISNULL(aggr_expr = select_stmt->get_aggr_item(0))) {
      ret = OB_ERR_UNEXPECTED;
      LOG_INFO("get unexpected null", K(ret));
    } else {
      for (int i = 0; i < select_stmt->get_aggr_item_size(); i++) {
        aggr_expr = select_stmt->get_aggr_item(i);
        aggr_param = aggr_expr->get_param_expr(0);
        if (OB_FAIL(copier.copy(aggr_param, new_aggr_param))) {
          LOG_INFO("masd failed to copy expr", K(ret));
          break;
        } else if (OB_FAIL(deep_copy_subquery_for_aggr(*view_child_stmt, new_aggr_param, aggr_expr->get_expr_type(), child_stmt))) {
          LOG_INFO("masd failed to deep copy subquery for aggr", K(ret));
          break;
        } else if (OB_FAIL(aggr_items.push_back(aggr_expr))) {
          LOG_INFO("masd failed to push back aggr item", K(ret));
          break;
        } else if (OB_FAIL(ctx_->expr_factory_->create_raw_expr(T_REF_QUERY, query_ref_expr))) {
          LOG_INFO("masd failed to create query ref expr", K(ret));
          break;
          ...
          ret = OB_ERR_UNEXPECTED;
          LOG_INFO("unexpected null expr or select item size of child stmt", K(ret));
        } else {
          ...
          if (OB_FAIL(query_ref_expr->add_column_type(target_expr->get_result_type()))) {
            LOG_INFO("add column type to subquery ref expr failed", K(ret));
          } else if (OB_FAIL(query_ref_expr->formalize(ctx_->session_info_))) {
            LOG_INFO("masd failed to formalize coalesce query expr", K(ret));
          } else if (OB_FAIL(query_ref_exprs.push_back(query_ref_expr))) {
            LOG_INFO("masd failed to push back query ref expr", K(ret));
          }
      }
    }}
    // } else if (OB_ISNULL(aggr_expr = select_stmt->get_aggr_item(0))
    ...
    // adjust select_stmt
    if (OB_FAIL(ret)) {
        LOG_INFO("masd failed to do transform min max", KPC(select_stmt));
    } else if (OB_FAIL(select_stmt->replace_relation_exprs(aggr_items, query_ref_exprs))) {
      LOG_INFO("masd failed to replace aggr exprs to query ref exprs", K(ret));
    } else if (OB_FAIL(select_stmt->get_condition_exprs().assign(select_stmt->get_having_exprs()))) {
      LOG_INFO("masd failed to assign condition exprs", K(ret));
    } else {
      ...
      if (OB_FAIL(select_stmt->adjust_subquery_list())) {
        LOG_INFO("masd failed to adjust subquery list", K(ret));
      } else if (OB_FAIL(select_stmt->formalize_stmt(ctx_->session_info_))) {
        LOG_INFO("masd failed to formalize stmt", K(ret));
      } else {
        LOG_INFO("succeed to do transform min max", KPC(select_stmt));
      }
    }
  }
  return ret;
                                                   ...
                                                   ObSelectStmt *&child_stmt)
{
  ...
  if (OB_ISNULL(ctx_) || OB_ISNULL(ctx_->expr_factory_) || OB_ISNULL(ctx_->stmt_factory_)
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_INFO("masd failed to find new aggr param", K(ret));
  ...
  if (OB_FAIL(ret)) {
                                                          ...
                                                          new_aggr_param,
                                                          child_stmt))) {
    LOG_INFO("masd failed to create select item", K(ret));
  } else if (OB_FAIL(set_child_condition(child_stmt, new_aggr_param))) {
    LOG_INFO("masd failed to set child condition", K(ret));
  } else if (OB_FAIL(set_child_order_item(child_stmt, new_aggr_param, aggr_type))) {
    LOG_INFO("masd failed to set child order item", K(ret));
  } else if (OB_FAIL(ObTransformUtils::set_limit_expr(child_stmt, ctx_))) {
    LOG_INFO("masd failed to set child limit item", K(ret));
  } else if (OB_FAIL(child_stmt->rebuild_tables_hash())) {
    LOG_INFO("masd failed to rebuild table hash", K(ret));
  } else if (OB_FAIL(child_stmt->update_column_item_rel_id())) {
    LOG_INFO("masd failed to update column item by id", K(ret));
                                           ...
                                           bool &is_valid)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && valid && i < stmt.get_aggr_item_size(); ++i) {
    if (OB_ISNULL(expr = stmt.get_aggr_item(i))) {
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_INFO("get unexpected null expr", K(ret));
...
int ObTransformMinMax::is_valid_select_list(const ObSelectStmt &stmt, bool &is_valid)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && valid && i < stmt.get_select_item_size(); ++i) {
    if (OB_FAIL(is_valid_select_expr(stmt.get_select_item(i).expr_, valid))) {
      LOG_INFO("masd failed to check expr is valid aggr", K(ret));
    }
  }
  is_valid = valid;
  return ret;
...
int ObTransformMinMax::is_valid_select_expr(const ObRawExpr *expr, bool &is_valid)
{
  ...
  if (OB_ISNULL(expr)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_INFO("expr should not be NULL", K(ret), KP(expr));
    ...
    for (int64_t i = 0; OB_SUCC(ret) && valid && i < expr->get_param_count(); ++i) {
      if (OB_FAIL(SMART_CALL(is_valid_select_expr(expr->get_param_expr(i), valid)))) {
        LOG_INFO("masd failed to check is_valid_expr", K(ret));
      }
    }
  } else {
    valid = false;
...
int ObTransformMinMax::is_valid_having_list(const ObSelectStmt &stmt, bool &is_valid)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && valid && i < stmt.get_having_expr_size(); ++i) {
    if (OB_FAIL(is_valid_having_expr(stmt.get_having_exprs().at(i), valid))) {
      LOG_INFO("masd failed to check having expr", K(ret), K(i));
    }
  }
  is_valid = valid;
  return ret;
...
int ObTransformMinMax::is_valid_having_expr(const ObRawExpr *expr, bool &is_valid)
{
  ...
  if (OB_ISNULL(expr)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_INFO("current expr is null", K(ret));
    ...
    for (int64_t i = 0; OB_SUCC(ret) && valid && i < expr->get_param_count(); ++i) {
      if (OB_FAIL(SMART_CALL(is_valid_having_expr(expr->get_param_expr(i), valid)))) {
        LOG_INFO("masd failed to check having expr", K(ret));
      }
    }
  }
  is_valid = valid;
  ...
  if (OB_ISNULL(expr)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && valid && i < expr->get_param_count(); ++i) {
      if (OB_FAIL(SMART_CALL(is_valid_order_expr(expr->get_param_expr(i), valid)))) {
        LOG_INFO("masd failed to check order expr", K(ret));
      }
    }
  }
  is_valid = valid;
                                             ...
                                             bool &is_valid)
{
  ...
  if (OB_ISNULL(stmt) || OB_ISNULL(expr)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_INFO("invalid argument", K(ret), K(stmt), K(expr));
  } else if (!expr->is_column_ref_expr()) {
    /* do nothing */
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_INFO("table item is null", K(ret));
    ...
    if (OB_FAIL(ObTransformUtils::is_match_index(ctx.sql_schema_guard_,
                                                 ...
                                                 &ctx))) {
      LOG_INFO("masd failed to check whether column matches index", K(ret));
                                            ...
                                            ObItemType aggr_type)
{
  ...
  if (OB_ISNULL(stmt) || OB_ISNULL(aggr_param)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_INFO("params have null", K(ret), K(stmt), K(aggr_param));
  } else {
    ...
    if (T_FUN_MAX == aggr_type) {
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_INFO("aggregate function type must by max or min", K(ret));
    }
    if (OB_SUCC(ret)) {
      if (OB_FAIL(stmt->add_order_item(new_order_item))) {
        LOG_INFO("masd failed to add order item", K(ret), K(stmt), K(new_order_item));
      }
    }
  }
  return ret;
